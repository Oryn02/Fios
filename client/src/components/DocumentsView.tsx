...
import {
  FileText, Sparkles, Loader2, BookOpen, MessageSquare, Send, Trash2, X, Bot, User, Upload, Folder,
} from 'lucide-react';
...
const DocumentsInner: React.FC = () => {
  ...
  const openTutor = (doc: FiosDocument) => {
    setActive(doc);
    setMessages([{ role: 'assistant', text: `Hi! Ask me anything about "${doc.title}". I'll answer based on your notes.` }]);
    setTutorOpen(true);
  };
  ...
  const send = async () => {
    ...
    setThinking(true);
    try {
      const { answer } = await askTutor(q, active.content || active.summary || '');
  ...
  return (
    <div className="space-y-6 max-w-6xl mx-auto font-sans text-[var(--fios-text)]">
      <div>
        ...
        <p className="text-xs font-mono text-[var(--fios-text-muted)] mt-1">Upload notes or PDFs to get an AI summary, a key-term glossary, and a grounded tutor chat.</p>
      </div>
      ...
      <div className="rounded-2xl border fios-border bg-[var(--fios-surface)] p-5 shadow-xl space-y-3">
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
          <input
            ...
            className="sm:col-span-2 bg-[var(--fios-surface-2)] border fios-border rounded-xl px-3.5 py-2.5 text-sm text-[var(--fios-text)] focus:outline-none focus:accent-border"
          />
      ...
      {active && (
        <div className="rounded-2xl border fios-border bg-[var(--fios-surface)] p-5 space-y-4">
          <div className="flex items-center justify-between">
            <h3 className="text-sm font-black uppercase flex items-center gap-2"><BookOpen className="w-4 h-4 accent-solid-text" /> {active.title}</h3>
              ...
              <div className="p-4 border-b fios-border flex items-center justify-between">
                <div className="flex items-center gap-2">
                  ...
                  <div>
                    ...
                    <p className="text-[10px] font-mono text-[var(--fios-text-muted)] truncate max-w-[240px]">{active.title}</p>
                  </div>
                </div>
